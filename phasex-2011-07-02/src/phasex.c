...
showusage(char *argvzero) {
    ...
    printf ("    -u, --uuid=         set a previously stored uuid, i.o. to restore a jack-session  \n");
...
//	jsession_uuid = NULL;
	...
	*cp++ = op->val;
	...
	case 'p':	/* ALSA MIDI ports */
	    ...
	    break;
	case 'u':
		/* uuid used by jack-session-handler for session restoration */
		jsession_uuid = strdup (optarg );
		break;
	case 'n':
