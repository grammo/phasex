...
jack_port_t			*input_port1 = NULL;
jack_port_t			*input_port2 = NULL;

jack_port_t			*output_port1 = NULL;
jack_port_t			*output_port2 = NULL;
...
/**
	called by session_callback (jack-session-handler callback) - from phasex/src/jack.c
	saves current banks
**/
int gui_session_cb( void *data )
{
	jack_session_event_t *ev = (jack_session_event_t *) data;
	char filename[128];		//  name.ext only
	char filepath[256];		//  /syspath/name.ext
	char command[256];		//  command to start phasex, incl. --uuid parameter  and statefile.phx
	char *tmp = NULL;
	char phasex_bin_dir[255] = "";								// could be "/usr/bin/" but leave emtpy as default
	//char phasex_bin_dir[255] = "/home/user/.../phasex/src/";		// set to build-dir for testing purposes
	
	
	/*
	// save banks too ?
	// unused code:
	tmp = get_patch_name_from_filename( user_bank_file );
	//
	if( strstr(user_bank_file, "jstate")==0 ){
		// jstate not found in user_bank_file
		// (this means, a new patch was loaded, with different name)
		
		// we append the patch-name to the filename
		snprintf( filename, sizeof(filename), "jstate_%s.phx", tmp );
	}else{
	
		// jstate was found, use the same filename
		snprintf( filename, sizeof(filename), "%s.phx", tmp );
	...
	*/
	...
	snprintf( filepath, sizeof(filepath), "%s%s", ev->session_dir, filename );
	
	
	// set user_bank_file to filepath, for saving
	// user_bank_file defined in phasex.h, used by save_bank() 
	snprintf (user_bank_file,         PATH_MAX, "%s",      filepath );
	
	// possible use (?):  USER_PATCH_DIR, user_data_dir
	
	snprintf( command,  sizeof(command),  "%sphasex  --uuid %s  ${SESSION_DIR}%s", 
				phasex_bin_dir,  ev->client_uuid,  filename );
	
	
	// -----------------------------------------------------------------------
	
	// now save   patch ("program")
	//if (!check_patch_overwrite (filename)) {
		int err = save_patch( filepath, program_number );	// in patch.c
		if (err == 0) {
			// no error
			//save_bank ();	// save the bank too ??
		}
	
	//}
	
	//on_patch_save_activate( NULL, filepath );
	//save_bank( );	// note: uses user_bank_file as filename
	
	// -----------------------------------------------------------------------
	
	// tell jackd the command line to start the application (phasex)
	ev->command_line = strdup(command);
	jack_session_reply( client, ev );	// client == jack_client
	
	// if a quit-event was requested, close phasex
...
/**
	jack-session-handler callback - forwards to gui_session_cb() above, via g_idle_add() method.
**/
...
jack_init(void) {
	...
	for( phasex_instance = 0; phasex_instance != 16; phasex_instance++)
	{
		...
		if( jsession_uuid != 0 ){
			// call with UUID
   	 		options = options | JackSessionID;
   	 		client = jack_client_open ((const char *)client_name, options, &status, server_name, jsession_uuid );
		} else {
			// without UUID
	...
	if( phasex_instance == 16 ){
			printf("jack_client_open() failed in jack_init(), phasex/src/jack.c ");
	}
